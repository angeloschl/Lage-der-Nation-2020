...
# Einleitung
In den folgenden Abbildungen habe ich das Podcastjahr 2020 der Lage der Nation konzentriert auf ein paar Aspekte veranschaulicht. Dabei habe ich zuerst kein konkretes Ziel verfolgt, sondern wollte die Zeit des Lockdowns etwas füllen. Herausgekommen sind aber ein paar spannende Erkenntnisse. Zum Beispiel, dass ich fast drei volle Tage in diesem Jahr die Lage gehört habe und dabei das Thema Corona um die 16 Stunden eingenommen hat. Oder das die Top-Quellen Süddeutsche, Spiegel und Twitter sind.
...
Wichtig ist mir zu sagen, dass ich keine Bewertung wie richtig - falsch oder zu viel - zu wenig mit dieser Auswertung machen möchte, sondern lediglich beschreiben will. Da ich es aber so interessant finde, möchte ich es gerne, auch in Rücksprache mit dem Lage-Team, hier im Forum teilen.

Ich gespannt, was ihr dazu sagt, ob ihr noch weiter Ideen für Abbildungen und Auswertungen habt und ob euch Fehler auffallen, die ich nicht gefunden habe. 
Den Code habe ich bei GitHub geteilt und könnt ihr hier einsehen: https://github.com/angeloschl/Lage-der-Nation-2020. Genutzt habe ich die Programmiersprache R und RMarkdown.

Ich habe mir die größte Mühe gemacht sauber zu arbeiten. Da ich programmieren und R aber nie für solche Zwecke gelernt habe und auch nur gelegentlich auf eine anderen Art und Weise im beruflichen Kontext nutze, bitte ich um Rücksicht bei Fehlern und nicht Einhaltung fundamentaler Regeln (sollte es diese Geben). Es ist mein erstes Projekt dieser Art und Größe. Ich wollte es nutzen, um meine Fertigkeiten etwas zu verbessern, neue Dinge zu lernen und einfach ein bisschen Spaß dabei zu haben. Über Feedback und Anregungen freue ich mich sehr.
...
# Abbildungen 
## Anzahl Folgen
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, dpi=300, fig.height = 3, fig.width = 9}
LDN_Jahr <- LDN %>%
...
ggsave("plots/Anzahl der Folgen.png",  width = 9, height = 3)
...
ggsave("plots/Länge der Folgen in 2020.png",  width = 9, height = 3)
...
Da die Kapitel oft unterschiedlich genannt wurden, aber die Inhalte gleich sind, wurden sie logisch zusammengefasst. <p>Hier am Beispiel das Kapitel <b>Corona</b> =
Covid, Corana, Corona Update, Coronavirus, Impfstoff, Impfung, Impfen, Impfpflicht, Immunitätsausweis <p>


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, dpi=300, fig.width = 9}
LDN_Kapitel <- LDN %>% 
  as_tibble() %>% 
...
ggsave("plots/Top 10 Kapitel in 2020 - Häufigkeit.png",  width = 9, height = 5)
...
ggsave("plots/Top 10 Kapitel in 2020 - Länge.png",  width = 9, height = 5)


```


```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, dpi=300, fig.width = 9, fig.height = 5}
LDN_Kapitel %>%
...
ggsave("plots/Top 8 Kapitel in 2020 - Pro Monat.png",  width = 9, height = 5)
...
ggsave("plots/Interviewgäste in 2020.png",  width = 9, height = 3)
...
ggsave("plots/Veröffentlichung nach Wochentagen in 2020.png",  width = 9, height = 5)
...
ggsave("plots/Top 10 Quellen 2020 - Häufigkeit.png",  width = 9, height = 5)
